...
$('#imageInput').addEventListener('change',e=>{for(const file of e.target.files){const total=state.files.reduce((n,x)=>n+x.file.size,0);if(file.size>2*1024*1024){$('#notice').textContent=`${file.name} 超过 2MB，未添加`;continue}if(total+file.size>3*1024*1024){$('#notice').textContent='图片合计不能超过 3MB，未添加其余图片';break}state.files.push({file,category:'其他材料',url:URL.createObjectURL(file)})}e.target.value='';renderFiles()});
